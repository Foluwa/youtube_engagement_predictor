...
if mode == "Fetch from YouTube":
    ...
    if st.button("Fetch & Predict"):
        try:
            ...
            st.video(video_url)
            st.image(res["thumbnail_url"], caption="Video Thumbnail", use_column_width=True)
    ...
    if st.button("Predict"):
        try:
            ...
            payload = {"title": title, "tags": tags, "publish_time": publish_time, "category_name": category_name}
...
if st.session_state.history:
    ...
    fig_time = px.line(df, x="publish_time", y=["pred", "actual"], labels={"value": "Like %", "variable": "Metric"})
    st.plotly_chart(fig_time, use_container_width=True)
    
    st.subheader("📊 Error Distribution")
    df_err = df.dropna(subset=["error"])
    fig_hist = px.histogram(df_err, x="error", nbins=20, labels={"error": "Prediction Error (%)"})
