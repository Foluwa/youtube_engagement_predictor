...
def run_monitoring(reference, current, model):
    ...
    ref_df = ref_X.copy()
    ref_df['target'], ref_df['prediction'] = reference['like_ratio'].values[:len(ref_df)], ref_preds
    curr_df = curr_X.copy()
    curr_df['target'], curr_df['prediction'] = current['like_ratio'].values[:len(curr_df)], curr_preds
    ...
    print(f"📊 Drift report saved to monitoring/drift_report.html")
